...
// inputs
  ...
  var CONCURRENCY = parseInt(process.argv[2], 10) || 10,
      ITERATIONS = parseInt(process.argv[3], 10) || 100000;
...
// hashes
  ...
  // 'djb2' algorithm
    ...
    var hash_djb2 = require('string-hash'),
        djb2 = function (ip) {
          return hash_djb2(ip) % CONCURRENCY;
        };
  ...
  // 'int31' algorithm from sticky-sessions
    ...
    var hash_int31 = function (ip, seed) {
        ...
        int31 = function (ip) {
          return hash_int31((ip || '').split(/\./g), seed) % CONCURRENCY;
        };
...
// get random ips
  ...
  var Chance = require('chance'), chance = new Chance(),
      ips = [], i = ITERATIONS;
  while (--i >= 0) ips.push((i % 2) ? chance.ip() : chance.ipv6());
...
// benchmarking
  ...
  function benchmark (ips, hash, concurrency, iterations) {
    ...
    var scatter = [], i = concurrency;
    while (--i >= 0) scatter.push(0);

    var index, timeStart, timeFinish;
    i = iterations;
